...
const AddProjectModal = ({ isModalOpen, closeModal, edit = false, id = null }) => {
    ...
    useEffect(() => {
        if (edit && isModalOpen) {
            axios.get(`${process.env.REACT_APP_SERVER}/project/${id}`)
                .then((res) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        if (!edit) {
            axios.post(`${process.env.REACT_APP_SERVER}/project/`, { title, description: desc })
                .then((res) => {
                    closeModal()
            ...
            axios.put(`${process.env.REACT_APP_SERVER}/project/${id}`, { title, description: desc })
                .then((res) => {
                    closeModal()
                    const customEvent = new CustomEvent('projectUpdate', { detail: { ...res.data } });
                    document.dispatchEvent(customEvent);
